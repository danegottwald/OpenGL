...
public:
    static bool IsKeyPressed(Key::KeyCode key) {
        int state = glfwGetKey(Window::Get().GetWindow(), static_cast<int>(key));
        return (state == GLFW_PRESS && state != GLFW_REPEAT);
    }

    static bool IsMousePressed(Mouse::MouseCode button) {
        int state = glfwGetMouseButton(Window::Get().GetWindow(), static_cast<int>(button));
        return (state == GLFW_PRESS || state == GLFW_REPEAT);
    }

    static glm::vec2 GetMousePos() {
        double x, y;
        glfwGetCursorPos(Window::Get().GetWindow(), &x, &y);
