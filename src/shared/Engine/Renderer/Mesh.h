...
public:
   ...
   void Compile() override
   {
      ...
      layout.Push< float >( 3 ); // Position
      layout.Push< float >( 3 ); // Normal
      layout.Push< float >( 3 ); // UV (xy = texture coords, z = layer)
      layout.Push< float >( 3 ); // Tint

      std::vector< float > vertices;
      vertices.reserve( 24 * ( 3 + 3 + 3 + 3 ) ); // 24 verts * 12 floats
         ...
         [ & ]( TextureAtlas::BlockFace face, const glm::vec3& normal, const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2, const glm::vec3& p3 )
      {
         ...
         const float                 layer  = static_cast< float >( region.layer );

         auto addVert = [ & ]( const glm::vec3& p, int uvIdx )
         {
            ...
            vertices.push_back( quadUVs[ uvIdx ].x );
            vertices.push_back( quadUVs[ uvIdx ].y );
